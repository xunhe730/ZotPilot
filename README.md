...
这个版本还把 MCP 工具从 33 个精简到 19 个，修了一批边界 bug，并加固了增量索引，让它在中断后能从断点正确恢复。
...
然后跟 agent 说「搜我库里关于 X 的论文」或「帮我调研 Y 方向」。它会按流程把 19 个 MCP 工具和 4 个 packaged skill 串起来完成任务。
...
| 组件 | 作用 |
|------|------|
| **MCP Server** | 19 个原子工具：语义搜索、引用图谱、入库、标签 / 集合 / 笔记管理 |
...
export SIMPLETEX_UAT=<your-token>          # 可选：公式 OCR（SimpleTex）
```
...
可选：开启 SimpleTex 公式 OCR 后，索引时会把识别出的 LaTeX 公式作为 `formula` chunk 写入 Chroma。该功能默认关闭，避免自动消耗 SimpleTex 额度；建议先用 `index_formulas` 小批量增量追加公式索引验证。

```bash
zotpilot config set formula_ocr_enabled true
zotpilot config set formula_ocr_api_key <SimpleTex-UAT>
zotpilot config set formula_ocr_max_formulas_per_doc 0   # 0 = no per-paper cap
```

已有索引不需要因为开启公式 OCR 而全量重建。对已建库论文，调用 MCP 工具 `index_formulas(item_keys=[...])` 只会刷新目标论文的 `formula` chunks，不会删除已有 `text` / `table` / `figure` chunks。

公式检索可以用 `search_papers(section_type="formulas")`，也可以用 `search_papers(chunk_types=["formula"])`。返回结果包含 `display_formula`（Markdown / MathJax display math）、`latex`（SimpleTex 原始 LaTeX）和 `image_path`（公式截图路径），方便同时做可读展示和人工核对。
...
<details>
<summary><b>MCP 工具列表（19 个）</b></summary>
...
| 引用 | `get_citations` |
| 索引 | `index_library`、`index_formulas`、`get_index_stats` |

`search_papers` 支持 `section_type` 参数搜表格 / 图表 / 公式（`tables`、`figures`、`formulas`）。公式结果会带 `display_formula`、`latex` 和 `image_path`。`ingest_by_identifiers` 接受 DOI / arXiv ID / URL 混合输入。
