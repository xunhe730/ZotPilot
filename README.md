...
具体来说，它在你本地的 Zotero 数据上建了一套向量索引，然后通过 MCP 协议暴露 24 个工具给 AI agent。AI 可以按意思搜论文（不是关键词匹配）、定位到具体章节段落、查谁引了谁、帮你打标签分类。论文数据不离开你的电脑。
...
<details>
<summary>写操作（5 个）</summary>

| 工具 | 说明 |
|------|------|
| `add_item_tags` / `remove_item_tags` | 加/删标签 |
| `set_item_tags` | 替换全部标签 |
| `add_to_collection` / `remove_from_collection` | 移进/移出文件夹 |
