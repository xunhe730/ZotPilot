...
## v0.5.3 — 公式索引 · 连接器入库增强 · 索引更稳

这个版本三条主线。

**公式语义索引（Phase A）**：本地 OCR 识别有文字层 PDF 中的 display 公式，写成可检索的公式 chunk，和正文 / 表格 / 图表一起参与语义检索。默认关闭、全程本地（需 `zotpilot[formula]` extra），首次联网下模型后可离线；inline / 纯图片公式留待后续阶段。

**连接器入库增强**：直链 PDF 与页面内嵌 iframe PDF 现在也能快速入库（无需 translator）；入库报错统一为错误码 + 中文可操作指引，PDF 抓取 / 反爬失败给「保持验证窗在前台、单篇重试」等针对性提示，并能一键跳回 Zotero 条目；预检不再为 translator 空等，可达性判断大幅提速。

**索引更稳**：修复 `batch_size>0` 关 vision 导致的「配置漂移」误报——以前会误导你 `force_reindex` 重建全部、白烧额度，现在引导用 `batch_size=0` 增量索引即可。
