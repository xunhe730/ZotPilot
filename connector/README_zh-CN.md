...
它是 [ZotPilot](https://github.com/xunhe730/ZotPilot) 的浏览器侧配套组件。真正的 MCP server、本地索引、Zotero API 写操作和 `save_urls()` 工具都在 ZotPilot 主仓库里。
...
这个目录不是独立产品，是 ZotPilot monorepo 中的浏览器侧组件，位于主仓库的 `connector/` 子目录下。

| 组件 | 路径 | 职责 |
|------|------|------|
| MCP server、搜索、索引、Zotero API 写操作 | `src/zotpilot/` | 面向 agent 的工具和编排层 |
| 浏览器执行层 | `connector/` | 打开页面、跑 translator、触发保存 |
| 本地 bridge | `src/zotpilot/` | 在 `127.0.0.1:2619` 上排队命令、收集结果 |

如果你只想要 Zotero 语义搜索，不需要安装 connector。

如果你想让 `save_urls()` 通过真实浏览器会话保存论文（含机构订阅 PDF），connector 是必需的浏览器侧组件。
