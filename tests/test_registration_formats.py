...
class TestClaudeCodeRegistration:
    def test_register_command_format(self):
        ...
        with (
            patch("zotpilot._platforms._runtime_invocation", return_value=("/usr/bin/zotpilot", ("mcp", "serve"))),
        ...
        assert mr.call_args_list[1].args[0] == [
            "claude",
            "mcp",
            "add",
            "--scope",
...
class TestOpenCodeInspectRoundtrip:
    def test_write_then_inspect_string(self, tmp_path, monkeypatch):
        from zotpilot._platforms import _inspect_registration, _write_mcp_config
        ...
        monkeypatch.setattr(
            _p, "_mcp_config_path", lambda plat: tmp_path / "opencode.json" if plat == "opencode" else None
        )
...
class TestEmbeddedSecretsDetection:
    def test_claude_no_secrets(self, tmp_path, monkeypatch):
        from zotpilot._platforms import CREDENTIAL_ENV_KEYS, _inspect_claude_registration
    ...
    def test_opencode_has_secrets(self, tmp_path, monkeypatch):
        from zotpilot._platforms import CREDENTIAL_ENV_KEYS, _inspect_registration
            ...
            json.dumps(
                {
                    "mcp": {
                        "zotpilot": {
                            "type": "local",
                            "command": ["/usr/bin/zotpilot", "mcp", "serve"],
                            "environment": {"GEMINI_API_KEY": "sk"},
                        }
                    }
                }
            )
    ...
    def test_claude_registered(self, tmp_path, monkeypatch):
        ...
        cp.write_text(
            json.dumps(
                {
                    "mcpServers": {
                        "zotpilot": {"type": "stdio", "command": "/usr/bin/zotpilot", "args": ["mcp", "serve"]}
                    }
                }
            )
        )
