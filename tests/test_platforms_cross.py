...
class TestPlatformDetectionMacOS:
    def test_detect_all_three_clients_mac(self, tmp_path, monkeypatch):
        ...
        with patch(
            "shutil.which",
            side_effect=_mock_which(
                {
                    "claude": str(bin_dir / "claude"),
                    "codex": str(bin_dir / "codex"),
                    "opencode": str(bin_dir / "opencode"),
                }
            ),
        ):
    ...
    def test_detect_partial_clients_mac(self):
        ...
        with (
            patch(
                "shutil.which",
                side_effect=_mock_which({"claude": "/usr/bin/claude", "codex": "/usr/bin/codex", "opencode": None}),
            ),
    ...
    def test_detect_opencode_via_package_json_mac(self, tmp_path, monkeypatch):
        ...
        with (
            patch("shutil.which", side_effect=_mock_which({"claude": None, "codex": None, "opencode": None})),
...
class TestPlatformDetectionWindows:
    def test_windows_detection_all_three(self):
        ...
        with patch(
            "shutil.which",
            side_effect=_mock_which(
                {"claude": "C:\\claude.cmd", "codex": "C:\\codex.cmd", "opencode": "C:\\opencode.cmd"}
            ),
        ):
            assert set(detect_platforms()) == {"claude-code", "codex", "opencode"}
    ...
    def test_windows_exe_binary_resolution(self):
        ...
        with (
            patch(
                "shutil.which",
                side_effect=_mock_which({"claude": "C:\\claude.cmd", "codex": "C:\\codex.cmd", "opencode": None}),
            ),
            patch("zotpilot._platforms._detect_app_install", return_value=False),
        ):
...
class TestConfigDataDirs:
    ...
    def test_windows_config_dir(self, monkeypatch):
        ...
        with patch.object(sys, "platform", "win32"):
            ...
            cfg = str(_default_config_dir())
            assert "AppData" in cfg and "Roaming" in cfg and "zotpilot" in cfg
            data = str(_default_data_dir())
            assert "AppData" in data and "Local" in data and "zotpilot" in data

    def test_linux_config_dir(self, monkeypatch):
        monkeypatch.setenv("HOME", "/home/testuser")
        from zotpilot.config import _default_config_dir
...
class TestZoteroDetector:
    def test_mac_profile_dir(self, monkeypatch, tmp_path):
        pd = tmp_path / "Library" / "Application Support" / "Zotero"
        pd.mkdir(parents=True)
        (pd / "profiles.ini").write_text("[Profile0]\nName=default\nIsRelative=1\nPath=abc.default\nDefault=1\n")
        (pd / "abc.default" / "prefs.js").parent.mkdir()
        (pd / "abc.default" / "prefs.js").write_text(
    ...
    def test_windows_profile_dir(self, monkeypatch, tmp_path):
        pd = tmp_path / "AppData" / "Roaming" / "Zotero" / "Zotero"
        pd.mkdir(parents=True)
        (pd / "profiles.ini").write_text("[Profile0]\nName=default\nIsRelative=1\nPath=abc.default\nDefault=1\n")
        (pd / "abc.default").mkdir()
        (pd / "abc.default" / "prefs.js").write_text(
            'user_pref("extensions.zotero.useDataDir", true);\nuser_pref("extensions.zotero.dataDir", "'
            + str(tmp_path / "Zotero")
            + '");\n'
    ...
    def test_fallback_to_home_zotero(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch("zotpilot.zotero_detector._detect_from_profiles", return_value=None),
        ):
            from zotpilot.zotero_detector import detect_zotero_data_dir
...
class TestMultiClientRegistration:
    def test_plan_changes_all_clean(self):
        from zotpilot._platforms import DesiredRuntime, PlatformRuntimeState, RuntimeState, plan_runtime_changes

        current = RuntimeState(
            package_version="0.5.0",
            supported_targets=("claude-code", "codex", "opencode"),
            platforms={
        ...
        desired = DesiredRuntime(
            command="/usr/bin/zotpilot", args=("mcp", "serve"), env={}, targets=("claude-code", "codex", "opencode")
        )
        ...
        current = RuntimeState(
            ...
            platforms={
                ...
                "codex": PlatformRuntimeState(
                    ...
                    skill_dirs=(),
                    skill_hash_ok=False,
                    registration_hash_ok=False,
                ),
            },
        )
        desired = DesiredRuntime(
            command="/usr/bin/zotpilot", args=("mcp", "serve"), env={}, targets=("claude-code", "codex")
        )
        c = plan_runtime_changes(desired, current)
...
class TestDoctorCrossPlatform:
    def test_config_permissions_mac_clean(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HOME", str(tmp_path))
        cp = tmp_path / ".config" / "zotpilot" / "config.json"
...
class TestUpgradeFlowCrossPlatform:
    def test_upgrade_uv_windows_lock_error(self, capsys):
        ...
        with (
            ...
            patch("zotpilot.cli._get_latest_pypi_version", return_value="0.5.1"),
            patch("zotpilot.cli._detect_cli_installer", return_value=("uv", ["uv"])),
            patch(
                "zotpilot.cli.subprocess.run",
                side_effect=subprocess.CalledProcessError(1, "uv", stderr="PermissionError: [WinError 32]"),
            ),
            patch("zotpilot.cli.sys") as ms,
        ):
            ms.platform = "win32"
            ms.executable = sys.executable
            ms.argv = sys.argv
...
class TestSkillDeploymentCrossPlatform:
    def _pp(self, fp):
        import zotpilot._platforms as m
    ...
    def test_deploy_mac_claude(self, tmp_path, monkeypatch):
        ...
        try:
            with (
                ...
                patch("zotpilot._platforms._skill_source_files", return_value=list(src.glob("*.md"))),
